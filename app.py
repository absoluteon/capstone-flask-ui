...
def index():
    ...
    ## Pie Chart 
    content_pie = df2.groupby('Content Rating').agg({'Installs' : 'sum'}). \
              rename({'Installs':'Pct of Total'}, axis=1). \
              sort_values('Pct of Total', ascending=False).head()
    fig = content_pie.plot(kind='pie', autopct='%1.1f%%', subplots = True, figsize=(5,5))
    plt.savefig('content_pie.png',bbox_inches="tight")
